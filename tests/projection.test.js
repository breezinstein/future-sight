...
describe('projectBucket', () => {
  ...
  it('annual compounding with monthly recurring deposits credits year-end interest on the accumulated balance', () => {
    const bucket = {
      id: 1, name: 'Test', currency: 'USD',
      starting_balance: 1000, expected_return: 0.10, compounding: 'annual',
      target_amount: null, target_date: null,
    ...
    const events = [
      { id: 1, scenario_id: 1, bucket_id: 1, type: 'deposit', amount: 100,
        date: '2025-01-01', recurring: 1, cadence: 'monthly', end_date: null,
        new_rate: null, escalation_rate: null, enabled: 1, notes: null },
    ];
  ...
  it('applies monthly recurring deposits', () => {
    const bucket = {
      id: 1, name: 'Test', currency: 'USD',
  ...
  it('supports semi-annual cadence (every 6 months)', () => {
    const bucket = {
      id: 1, name: 'Test', currency: 'USD',
      starting_balance: 0, expected_return: 0, compounding: 'monthly',
      target_amount: null, target_date: null,
    ...
    const events = [
      { id: 1, scenario_id: 1, bucket_id: 1, type: 'deposit', amount: 1000,
        date: '2025-01-01', recurring: 1, cadence: 'semi_annual', end_date: null,
        new_rate: null, escalation_rate: null, enabled: 1, notes: null },
    ];
    const series = projectBucket(bucket, events, start, 24);
    // Deposits fire at months 0, 6, 12, 18, 24 within the projection range.
    // First step is i=1, so triggers fall on months 6, 12, 18, 24 = 4 deposits.
    expect(series[24].balance).toBeCloseTo(4000);
  });

  it('supports biennial cadence (every 2 years)', () => {
    const bucket = {
      id: 1, name: 'Test', currency: 'USD',
      starting_balance: 0, expected_return: 0, compounding: 'monthly',
      target_amount: null, target_date: null,
    ...
    const events = [
      { id: 1, scenario_id: 1, bucket_id: 1, type: 'deposit', amount: 5000,
        date: '2025-01-01', recurring: 1, cadence: 'biennial', end_date: null,
        new_rate: null, escalation_rate: null, enabled: 1, notes: null },
    ];
    const series = projectBucket(bucket, events, start, 60);
    // Deposits at months 24 and 48 (within projection 1..60) = 2 deposits.
    expect(series[60].balance).toBeCloseTo(10000);
  });
